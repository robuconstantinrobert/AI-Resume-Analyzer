...
def get_embedding_model():
    from sentence_transformers import SentenceTransformer
    # Model is loaded only in function scope, not globally
    model = SentenceTransformer("all-MiniLM-L6-v2")
    return model

def embed_chunks(chunks):
    # Only instantiate model when needed
    model = get_embedding_model()
...
def process_resume_file(file_bytes: bytes, filename: str):
    ...
    # Import here to avoid module RAM use
    from langchain.text_splitter import RecursiveCharacterTextSplitter
    from db import insert_chunks_bulk
    ...
    result = []
    # Batch size of 1 for minimal memory
